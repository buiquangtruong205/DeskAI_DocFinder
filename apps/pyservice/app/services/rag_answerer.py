...
5. If no relevant documents are provided, respond: "Không có tài liệu liên quan nào được cung cấp." (Vietnamese) or "No relevant documents provided." (English)
6. Always respond in Vietnamese if the question is in Vietnamese""",
...
4. If no documents provided, respond: "Không có tài liệu nào để tóm tắt."
5. Respond in Vietnamese if documents are in Vietnamese""",
...
4. If no documents provided, respond: "Không có tài liệu nào để giải thích khái niệm này."
...
def generate_follow_ups(question: str, answer: str) -> List[str]:
    ...
    # Don't generate follow-ups if no documents were found
    if "không có tài liệu" in answer.lower() or "no documents" in answer.lower() or "không có thông tin" in answer.lower():
        return []
    
    # Generate contextual follow-ups based on the question and answer
    if "how" not in question.lower() and "làm thế nào" not in question.lower():
        follow_ups.append("Làm thế nào để thực hiện điều này?" if "tài liệu" in answer else "How does this work in practice?")
    if "example" not in answer.lower() and "ví dụ" not in answer.lower():
        follow_ups.append("Bạn có thể cho tôi một ví dụ?" if "tài liệu" in answer else "Can you show me an example?")
    if len(answer) > 500:
        follow_ups.append("Bạn có thể tóm tắt các điểm chính?" if "tài liệu" in answer else "Can you summarize the key points?")
